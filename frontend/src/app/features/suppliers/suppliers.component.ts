  ...
  template: `
    <div class="page">
      <div class="page-header">
        <h1>Suppliers</h1>
        <button type="button" class="btn-primary" (click)="openCreateModal()">+ New Supplier</button>
      </div>
          ...
          <tbody>
            @for (item of items; track item.id) {
              <tr>
                <td>{{ item.name }}</td>
                <td>{{ item.contact || '-' }}</td>
                <td>{{ item.rating != null ? (item.rating | number:'1.1-1') : '-' }}</td>
                <td>{{ item.leadTime != null ? item.leadTime : '-' }}</td>
              ...
              <tr><td colspan="6" class="empty">No suppliers found</td></tr>
            }
          </tbody>
        </table>
      </div>
    ...
    @if (showModal) {
      <div class="modal-backdrop" (click)="closeModal()">
        <div class="modal" (click)="$event.stopPropagation()">
          <h2>{{ editId ? 'Edit' : 'New' }} Supplier</h2>
          <form (ngSubmit)="save()" #f="ngForm">
            ...
            <div class="field">
              <label>Contact</label>
              <input [(ngModel)]="form.contact" name="contact" placeholder="Email or phone" />
            </div>
            <div class="field-row">
              <div class="field">
                <label>Rating (0-5)</label>
                <input type="number" [(ngModel)]="form.rating" name="rating" min="0" max="5" step="0.1" placeholder="0-5" />
              </div>
              <div class="field">
                <label>Lead Time (days)</label>
                <input type="number" [(ngModel)]="form.leadTime" name="leadTime" min="0" placeholder="0" />
              </div>
            </div>
...
export class SuppliersComponent implements OnInit {
  ...
  form: SupplierForm = { name: '', contact: '', rating: null, leadTime: 0 };
  saving = false;
  modalError = '';
  ...
  openCreateModal(): void {
    this.editId = null;
    this.form = { name: '', contact: '', rating: null, leadTime: 0 };
    this.modalError = '';
    this.showModal = true;
  }
  ...
  save(): void {
    ...
    const ratingVal = this.form.rating as number | string | null | undefined;
    const numRating = ratingVal != null && ratingVal !== '' && !isNaN(Number(ratingVal)) ? Number(ratingVal) : null;
    const payload: SupplierForm = {
      name: this.form.name,
      contact: this.form.contact || '',
      rating: numRating,
      leadTime: Math.max(0, Number(this.form.leadTime) || 0),
    };
